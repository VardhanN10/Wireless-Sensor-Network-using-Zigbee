...
/***************************************************************************//**
 * @brief Endpoints definitions and variables
 ******************************************************************************/
#define SWITCH1_ENDPOINT 1
                                        ...
                                        uint8_t* value)
{
  ...
  if (clusterId == ZCL_ON_OFF_CLUSTER_ID
    ...
    if (endpoint == SWITCH1_ENDPOINT) {
      // Read the current value of the On/Off attribute from the server
      if (emberAfReadServerAttribute(SWITCH1_ENDPOINT,
    ...
    if (endpoint == SWITCH2_ENDPOINT) {
      if (emberAfReadServerAttribute(SWITCH2_ENDPOINT,
                                     ZCL_ON_OFF_CLUSTER_ID,
                                     ZCL_ON_OFF_ATTRIBUTE_ID,
                                     (uint8_t *)&onOff,
                                     sizeof(onOff))
        ...
        // Control LED0 based on the attribute value
        if (onOff) {
          sl_led_turn_on(&sl_led_led1);  // Turn on LED0 when attribute is ON (true)
        } else {
          sl_led_turn_off(&sl_led_led1); // Turn off LED0 when attribute is OFF (false)
        }
      }
    }
  }
